...
export const Card = memo(
	forwardRef<React.ElementRef<typeof View>, React.ComponentPropsWithoutRef<typeof View> & { rootClassName?: string }>(
		({ className, rootClassName, ...props }, ref) => (
			<View className={cn("bg-card ios:shadow-xl ios:rounded-2xl rounded-xl shadow-2xl", rootClassName)}>
				<View
					ref={ref}
...
export const CardBadge = memo(
	forwardRef<React.ElementRef<typeof View>, React.ComponentPropsWithoutRef<typeof View>>(({ className, style, ...props }, ref) => {
...
export const CardTitle = memo(({ className, ...props }: React.ComponentPropsWithoutRef<typeof Text>) => {
	return (
		<Text
			role="heading"
			aria-level={3}
			className={cn("text-card-foreground ios:font-bold text-3xl font-medium leading-none tracking-tight", className)}
			{...props}
		/>
	)
})
...
export const CardSubtitle = memo(
	({ className, variant = Platform.select({ ios: "footnote" }), ...props }: React.ComponentPropsWithoutRef<typeof Text>) => {
		return (
			<Text
				variant={variant}
...
export const CardDescription = memo(({ className, ...props }: React.ComponentPropsWithoutRef<typeof Text>) => {
	return (
		<Text
...
export function addOpacityToRgb(rgb: string, opacity: number): string {
	...
	if ([r, g, b].some(value => (value ?? -1) < 0 || (value ?? -1) > 255)) {
