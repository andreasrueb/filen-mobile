...
async function socketEventListener(socketEvent: SocketEvent, focusedChatUUID: string | null) {
	try {
...
export const SocketEvents = memo(() => {
	...
	useEffect(() => {
		const listener = events.subscribe("socketEvent", event => socketEventListener(event, focusedChatUUID))
